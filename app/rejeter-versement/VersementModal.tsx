...
export default function VersementModal({ onClose, onAdd }: VersementModalProps) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const res = await fetch(`${APP_URL}/api/versement/create`, {
        ...
        body: JSON.stringify({ utilisateurId: Number(utilisateurId), montant: Number(form.montant), description: form.description }),
      });
      ...
      if (res.ok) { showNotification(message, "success"); onAdd(); }
      else showNotification(message, "error");

    } catch (error) { console.error(error); showNotification("Erreur serveur. Veuillez réessayer.", "error"); }
        ...
        <div className="modal-content">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="modal-buttons">
              <button type="button" onClick={onClose} className="cancel-btn" disabled={loading}>Annuler</button>
